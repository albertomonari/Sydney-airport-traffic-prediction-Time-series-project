...
# In-sample analysis 
# We test the predictive power of our model by subsetting our data in two sets: 
# data.train to build our model and data.test to test its accuracy  
data.train <- window(ltraf,start=c(2009,1), end=c(2018,12))

data.test <- window(ltraf,start=c(2019,1), end=c(2019,12))

mod.train <- arima(data.train,c(0,1,1), seasonal=list(order=c(0,1,1), period=12), method='ML')
mod.train
pred.test <- predict(mod.train,n.ahead=12)

# Plot of the prediction with the training model
values_predicted.test <- 2.718^pred.test$pred
values_predicted.test
ts.plot(traffic.ts,2.718^pred.test$pred, log = "y", lty = c(1,3))
time(traffic.ts)
# Graph with all years
ts.plot(traffic.ts,xlim=c(2009,2020), main = "Forecasting in-sample")
lines(2.718^(pred.test$pred), col="red")
lines(2.718^(pred.test$pred-1.96*pred.test$se),col=4,lty=2)
lines(2.718^(pred.test$pred+1.96*pred.test$se),col=4,lty=2)
# Graph focused on the train set (2019)
ts.plot(traffic.ts,xlim=c(2019,2019.9), main = "Forecasting in-sample")
lines(2.718^(pred.test$pred), col="red")
lines(2.718^(pred.test$pred-1.96*pred.test$se),col=4,lty=2)
lines(2.718^(pred.test$pred+1.96*pred.test$se),col=4,lty=2)
...
# We have an overall good accuracy with very low values for mean error (ME), 
# root mean squared error (RMSE), mean absolute error (MAE), mean percentage error (MPE), 
# mean absolute percentage error (MAPE), mean absolute scaled error (MASE) and 
# the first-order autocorrelation coefficient.


# Out-of-sample analysis
# We use our model to forecast the traffic for the year 2020
#?predict
# Prediction of 12 months (all year 2020) after the latest month (December 2019)
pred <- predict(mod1, n.ahead = 12)
values_predicted <- 2.718^pred$pred
values_predicted
ts.plot(traffic.ts,2.718^pred$pred, log = "y", lty = c(1,3))
time(traffic.ts)
ts.plot(traffic.ts,xlim=c(2009,2020.500), main = "Forecasting out-of-sample")
